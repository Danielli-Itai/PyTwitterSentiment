...
# In other words, the model learnt patterns specific to the training data, which are irrelevant in other data.
# 
# We can identify overfitting by looking at validation metrics, like loss or accuracy.
# Usually the validation metric stops improving after a certain number of epochs and begins to decrease afterwards.
# The training metric continues to improve because the model seeks to find the best fit for the training data.
# 
# There are several manners in which we can reduce overfitting in deep learning models.
# The best option is to ***get more training data***.
# Unfortunately, in real-world situations you often do not have this possibility due to time, budget or technical constraints.
# 
# Another way to reduce overfitting is to ***lower the capacity of the model to memorize the training data***.
# As such, the model will need to focus on the relevant patterns in the training data, which results in better generalization. In this post we'll discuss three options to achieve this.

# # Set-up of the project
# We start by importing the necessary packages and configuring some parameters.
# We will use  [Keras](https://keras.io/) to fit the deep learning models.
# The training data is the [Twitter US Airline Sentiment data set from Kaggle](http:// https://www.kaggle.com/crowdflower/twitter-airline-sentiment).
