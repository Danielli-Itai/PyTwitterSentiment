...
# # Introduction
# In my previous kernel, I explored deep learning with Keras on the TwitterAirline data set of Kaggle.
# The model with Dropout layers produced the best results.
# However, it was still outperformed by the LogisticRegression model.
# Let's try if we can do better than the LogisticRegression with a deep learning model that uses word embeddings.
# 
...
# # Word Embeddings
# When applying one-hot encoding to the words in the tweets, we end up with sparse vectors of high dimensionality (here the number of words).
# On larger data sets this could cause performance issues. Additionally, one-hot encoding does not take into account the semantics of the words. For instance, *plane* and *aircraft* are different words but have a similar meaning.
# 
